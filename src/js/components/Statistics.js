...
export class Statistics {
  constructor({ arr, query, month, container, from, to }) {
    this.arr = arr;
    this.query = query;
    this.month = month;
  ...
  _groupNewsDay() {
    const arr = [];
    const differenceDate = (this.to - this.from) / 86400000;
    for (let days = 0; days <= differenceDate; days++) {
      const statisticDay = determineDate(this.to, days);
    ...
    this.arr.forEach((item) => {
      ...
      let dateStatistics = arr.find(
        (elem) => Date.parse(elem.date) === Date.parse(publishedDay)
      );
      if (dateStatistics) {
  ...
  _calculateStatistics(arr) {
    arr.forEach((element) => {
      element.keywordCount += getNumberResultsArticle(
        this.query,
        element.articles,
        FIELD_TITLE
      );
  ...
  renderStatistic() {
    const arr = this._calculateStatistics(this._groupNewsDay());
